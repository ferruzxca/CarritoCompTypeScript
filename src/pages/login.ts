...
function render() {
  ...
  // Si hay sesión, NO redirige. Muestra opciones.
  if (existing) {
    hint.style.display = "block";
    hint.innerHTML = `
      <div class="panel" style="background: rgba(255,255,255,.04); border-style:dashed">
        Ya hay sesión como <strong>${existing.email}</strong>.
    ...
    hint.querySelector<HTMLButtonElement>("#btn-continue")!
        .addEventListener("click", () => { window.location.href = "products.html"; });
